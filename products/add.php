...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($name) && !empty($price)) {
        $stmt = $conn->prepare("INSERT INTO products (name, barcode, sku, category_id, supplier_id, cost, price, stock, unit, image, low_stock_alert) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$name, $barcode, $sku, $category_id, $supplier_id, $cost, $price, $stock, $unit, $image_name, $low_stock_alert]);
